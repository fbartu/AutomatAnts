...
for j in range(run_steps):
	...
	#Prints while runing
	if (j % 1 == 0):
		print("Step: ", int(j/1), "etta: ", etta, "mean: ",np.mean(E_count_array))
	if (j == run_steps-1):
...
#Save States data
DataOut_eta = np.column_stack((eta_list,n_mean_list))
np.savetxt(params.file_name+"_eta.dat",DataOut_eta)
